...
<div class="mfeedback">
    ...
    <form class="mfeedback-form" action="<?= POST_FORM_ACTION_URI ?>" method="POST">
        <?= bitrix_sessid_post() ?>
        ...
        <div class="mf-themes">
            <div class="mf-text">
                Выберите тему
                <?php if (empty($arParams["REQUIRED_FIELDS"]) || in_array("THEMES", $arParams["REQUIRED_FIELDS"])): ?>
                    <span class="mf-req">*</span>
                <?php endif ?>
            </div>
            <select name="THEME">
                <?php foreach ($arParams['THEMES'] as $themeId => $themeName): ?>
                    <option value="<?= htmlspecialchars($themeName) ?>"><?= htmlspecialchars($themeName) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
