...
<div class="bread_crumbs">
    <a class="bread_crumbs_start" href="/books/">Главная /</a>
...
<div class="item_info_section">
    ...
    $properties = ['AUTHOR', 'GENRE', 'BINDING'];
    foreach ($properties as $property) {
        if (!empty($arResult['PROPERTIES'][$property]['VALUE'])) {
            ?>
            <div class="bx_item_property">
                <strong><?php echo $arResult['PROPERTIES'][$property]['NAME']; ?>:</strong>
                <?php
                $value = $arResult['PROPERTIES'][$property]['VALUE'];
                if ($property === 'AUTHOR') {
                    // Если свойство AUTHOR, получаем его название по ID
                    if (is_array($value)) {
                        $authorNames = [];
                        foreach ($value as $authorId) {
                            $res = CIBlockElement::GetByID($authorId);
                            if ($ar_res = $res->GetNext()) {
                                $authorNames[] = $ar_res['NAME'];
                            }
                        }
                        ...
                        if ($ar_res = $res->GetNext()) {
                            ?>
                            <p><?php echo $ar_res['NAME']; ?></p>
                            <?php
                        }
                    }
                } elseif ($property === 'GENRE') {
                    if (is_array($value)) {
                        foreach ($value as $genre) {
                            ?>
                            <a href="/books/?GENRE=<?php echo urlencode($genre); ?>"><?php echo $genre; ?></a><br>
                            <?php
                        }
                    } else {
                        ?>
                        <a href="/books/?GENRE=<?php echo urlencode($value); ?>"><?php echo $value; ?></a><br>
                        <?php
                    }
                } else {
                    if (is_array($value)) {
